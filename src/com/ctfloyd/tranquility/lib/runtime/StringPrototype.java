...
public class StringPrototype extends JsObject {
    ...
    private Value split(ArgumentList arguments) {
        Value unknown = getRuntime().getThisValue();
        ...
        if (limit == 0) {
            return Value.object(ArrayObject.create(getRealm(), Collections.emptyList()));
        }

        // 7. If separator is undefined, then
        if (separator.equals("undefined")) {
            return Value.object(ArrayObject.create(getRealm(), Collections.singletonList(Value.string(string))));
        }
        ...
        if (separatorLength == 0) {
            ...
            return Value.object(ArrayObject.create(getRealm(),codeUnits));
        }
        ...
        while (j != -1) {
            ...
            if (substrings.size() == limit) {
                return Value.object(ArrayObject.create(getRealm(), substrings));
            }
    ...
    private Value at(ArgumentList argumentList) {
    ...
    private Value charAt(ArgumentList arguments) {
        // 1.  Let O be the RequireObjectCoercible (this value).
        Value unknown = getRuntime().getThisValue();
        ASSERT(unknown.isObject());
        ASSERT(unknown.asObject().isStringObject());
    ...
    private Value endsWith(ArgumentList arguments) {
        // 1.  Let O be the RequireObjectCoercible (this value).
        Value unknown = getRuntime().getThisValue();
        ASSERT(unknown.isObject());
        ASSERT(unknown.asObject().isStringObject());
    ...
    private Value includes(ArgumentList arguments) {
        // 1.  Let O be the RequireObjectCoercible (this value).
        Value unknown = getRuntime().getThisValue();
        ASSERT(unknown.isObject());
        ASSERT(unknown.asObject().isStringObject());
